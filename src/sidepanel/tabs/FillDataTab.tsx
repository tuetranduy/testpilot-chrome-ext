...
export function FillDataTab({ tab, settings, siteRecord, onUpdate }: Props) {
  ...
  const selectableFields = formFields.filter((field) => siteRecord.fieldValues[field.id] !== undefined)
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      const providerImages = getVisionCapability(settings.activeProvider, config) === 'vision'
        ? webScan.images.map((image) => image.dataUrl)
        : []
      const text = await chatWithProvider(settings.activeProvider, config, [
        { role: 'system', content: system },
        { role: 'user', content: user },
      ], { temperature: 0.9, ...(providerImages.length > 0 ? { images: providerImages } : {}) })
  ...
  return (
    <div className="flex flex-col gap-3.5">
      <Card>
        ...
        <p className="mt-1.5 text-xs leading-5 text-muted">Generate realistic values, review them, then select fields to fill or fill one field at a time.</p>
        {!siteRecord.lastScan ? (
          ...
          <>
            <div className="mt-3 grid grid-cols-2 gap-2">
              ...
              <Button variant="secondary" onClick={fillSelected} disabled={filling !== null || selectedIds.size === 0}>
                {filling === 'all' ? <Spinner /> : `Fill selected (${selectedIds.size})`}
              </Button>
            </div>
            {error && <div className="mt-3"><InlineMessage tone="error">{error}</InlineMessage></div>}

            <div className="mt-3 flex items-center gap-2 border-t border-border pt-3">
              <Button variant="ghost" size="small" onClick={() => setSelectedIds(new Set(selectableFields.map((field) => field.id)))} disabled={filling !== null || selectableFields.length === 0}>
                Select all
              </Button>
              <Button variant="ghost" size="small" onClick={() => setSelectedIds(new Set())} disabled={filling !== null || selectedIds.size === 0}>
                Deselect all
              </Button>
            </div>
            ...
            <ul className="mt-2 flex flex-col gap-2.5">
              {formFields.map((field) => {
                ...
                const hasValue = siteRecord.fieldValues[field.id] !== undefined
                return (
                  <li key={field.id} className="flex items-end gap-2 rounded-xl border border-border bg-bg/35 p-2.5">
                    <input
                      ...
                      disabled={filling !== null || !hasValue}
                      className="mb-3 h-4 w-4 shrink-0 accent-[var(--color-cta)]"
                    />
                    <div className="min-w-0 flex-1">
                      <label htmlFor={`field-${field.id}`} title={label} className="mb-1 block truncate text-[11px] font-semibold text-muted">
                        {label}
                      </label>
                      <input
                        id={`field-${field.id}`}
